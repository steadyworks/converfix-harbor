...
calibrated = CalibratedClassifierCV(
    estimator=LinearSVC(C=1.0, class_weight=None, max_iter=200, tol=1e-3, dual=True),
    cv=5, method="sigmoid"
)
...
thr_candidates = np.linspace(0.1, 0.3, 33)
...
def best_threshold_cv(model, X_df, y, thresholds, cv):
    ...
    for tr_idx, va_idx in cv.split(X_df, y):
        ...
        for i, t in enumerate(thresholds):
            ...
            scores[i] += f1_score(preds, y_va)
    scores /= cv.get_n_splits()
    return thresholds[np.argmin(scores)], scores
