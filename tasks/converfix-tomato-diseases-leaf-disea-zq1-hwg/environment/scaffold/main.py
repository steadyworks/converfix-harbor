...
if not os.path.exists(TRAIN_FOLDER):
    ...
    train_split, val_split = train_test_split(train_csv, test_size=0.15, random_state=42)
...
print("\n❄️ Phase 2: Unfreezing top 10 layers for Deep Fine-Tuning...")

base_eff.trainable = True
# Freeze the bottom layers (Keep structural knowledge)
# Unfreeze the top 10 layers (Learn Tomato specifics)
for layer in base_eff.layers[:-10]:
    layer.trainable = False

# Re-compile with adjusted Learning Rate for fine-tuning
model_eff.compile(
