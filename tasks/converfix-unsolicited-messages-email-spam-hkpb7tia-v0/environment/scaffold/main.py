...
encoder.fit(train_df['Category'])  # fit on train labels so we can inverse_transform later
train_df['target'] = encoder.transform(train_df['Category'])

# Remove duplicates in training data
train_df = train_df.drop_duplicates(subset=['Message'], keep='first')
...
X_test = tfid.transform(test_df['transformed_text']).toarray()
...
clfs = {
    'SVC': SVC(kernel="sigmoid", gamma=0.001, probability=True),
    ...
    'xgb': XGBClassifier(n_estimators=5, random_state=2, use_label_encoder=False, eval_metric='logloss'),
}
